...
async def api_mark_recovered(page_key: str, recovered_at: str) -> bool:
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{D1_WORKER_URL}/tracked/update",
                headers=_d1_headers(),
                json={"username": page_key, "recovered_at": recovered_at},
            ) as resp:
                if resp.status != 200:
                    body = (await resp.text())[:300]
                    print(f"api_mark_recovered({page_key}) failed: HTTP {resp.status} — {body}", flush=True)
                    return False
                return True
    except (aiohttp.ClientError, asyncio.TimeoutError) as e:
        print(f"api_mark_recovered({page_key}) failed: {type(e).__name__}: {e}", flush=True)
        return False


async def api_mark_banned(page_key: str, banned_at: str) -> bool:
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{D1_WORKER_URL}/tracked/update",
                headers=_d1_headers(),
                json={"username": page_key, "banned_at": banned_at},
            ) as resp:
                if resp.status != 200:
                    body = (await resp.text())[:300]
                    print(f"api_mark_banned({page_key}) failed: HTTP {resp.status} — {body}", flush=True)
                    return False
                return True
    except (aiohttp.ClientError, asyncio.TimeoutError) as e:
        print(f"api_mark_banned({page_key}) failed: {type(e).__name__}: {e}", flush=True)
        return False


async def api_set_fail_count(page_key: str, fail_count: int) -> bool:
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{D1_WORKER_URL}/tracked/update",
                headers=_d1_headers(),
                json={"username": page_key, "fail_count": fail_count},
            ) as resp:
                if resp.status != 200:
                    body = (await resp.text())[:300]
                    print(f"api_set_fail_count({page_key}) failed: HTTP {resp.status} — {body}", flush=True)
                    return False
                return True
    except (aiohttp.ClientError, asyncio.TimeoutError) as e:
        print(f"api_set_fail_count({page_key}) failed: {type(e).__name__}: {e}", flush=True)
        return False


async def api_remove_tracked(page_key: str) -> bool:
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{D1_WORKER_URL}/tracked/delete",
                headers=_d1_headers(),
                json={"username": page_key},
            ) as resp:
                if resp.status != 200:
                    body = (await resp.text())[:300]
                    print(f"api_remove_tracked({page_key}) failed: HTTP {resp.status} — {body}", flush=True)
                    return False
                return True
    except (aiohttp.ClientError, asyncio.TimeoutError) as e:
        print(f"api_remove_tracked({page_key}) failed: {type(e).__name__}: {e}", flush=True)
        return False
...
def build_recovery_embed(info: dict, start_iso: str) -> discord.Embed:
    """Clean, single-block recovery notification — no duplicate stats."""
...
def build_ban_embed(page_key: str, start_iso: str) -> discord.Embed:
    ...
    description += f"⏱️ *Banned after: {format_elapsed_long(start_iso)}*"
...
async def check_tracked_pages():
    try:
        ...
        print(
            f"check_tracked_pages: {pending_count} page(s) need checking, "
        ...
        for page_key, meta in tracked.items():
            ...
            # Skip already-finished entries entirely — no point burning an
            # Apify call on a page that's already been marked banned/recovered
            # and will never notify again anyway.
            ...
            if track_type == "ban":
                if info is None and not meta.get("banned", False):
                    # Require 2 consecutive unreachable checks before declaring
                    # a real ban — a single miss is often just a privacy
                    # setting (personal profiles can be friends-only, which
                    # isn't a ban) or a transient Apify hiccup, not proof of
                    # an actual ban/restriction.
                    fail_count = meta.get("fail_count", 0) + 1
                    if fail_count >= 2:
                        embed = build_ban_embed(page_key, meta["start_time"])
                        await channel.send(embed=embed)
                        ok = await api_mark_banned(page_key, datetime.now(timezone.utc).isoformat())
                        if ok:
                            print(f"🚫 Banned (confirmed after {fail_count} checks): {page_key}", flush=True)
                        else:
                            print(f"⚠️ Sent ban notification for {page_key} but failed to save banned=1 to database — will re-notify next check.", flush=True)
                    else:
                        await api_set_fail_count(page_key, fail_count)
                        print(f"⏳ {page_key} unreachable (check {fail_count}/2) — waiting for confirmation before declaring banned.", flush=True)
                elif info is not None and meta.get("fail_count", 0) > 0:
                    # Page is reachable again after a prior miss — reset the
                    # counter, it wasn't actually going down.
                    await api_set_fail_count(page_key, 0)
                ...
                if info is not None and not meta.get("recovered", False):
                    ...
                    await channel.send(embed=embed)
                    ok = await api_mark_recovered(page_key, datetime.now(timezone.utc).isoformat())
                    if ok:
                        print(f"✅ Recovered: {page_key}", flush=True)
                    else:
                        print(f"⚠️ Sent recovery notification for {page_key} but failed to save recovered=1 to database — will re-notify next check.", flush=True)
...
async def untrack(interaction: discord.Interaction, pageurl: str):
    ...
    if page_key in tracked:
        ok = await api_remove_tracked(page_key)
        if ok:
            embed = discord.Embed(
            ...
            await interaction.response.send_message(
                f"❌ Failed to remove `{page_key}` — the database delete did not succeed. "
