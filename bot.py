...
async def api_clear_tracked(page_key: str):
    ...
    try:
        ...
        for page_key, meta in tracked.items():
            ...
            if track_type == "ban" and meta.get("banned", False):
                continue  # Already banned, skip checking
            if track_type == "recovery" and meta.get("recovered", False):
                continue  # Already recovered, skip checking
            
            info = await check_facebook_status(page_key)
            
            if track_type == "ban":
                # ONLY send ban notification if page is NOT reachable AND not already marked as banned
                ...
                # ONLY send recovery notification if page IS reachable AND not already marked as recovered
...
async def unban(interaction: discord.Interaction, pageurl: str):
    ...
    tracked = await api_get_tracked()
    if page_key in tracked:
        # Remove completely from tracking
        await api_remove_tracked(page_key)
        display_name = get_display_name(page_key)
        embed = discord.Embed(
