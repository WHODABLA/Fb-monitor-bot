...
async def check_facebook_status(page_key: str):
    ...
    *** Field names below are UNVERIFIED — confirm against a real test run
    in Apify's console before trusting this in production. See the module
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if not data or not isinstance(data, list) or len(data) == 0:
                    print(f"[apify-fb] {page_key}: 200 OK but empty dataset — {data}", flush=True)
                    return None
                item = data[0]
                if item.get("error") or not (item.get("title") or item.get("pageId")):
                ...
                return {
                    ...
                    "profile_pic_url": item.get("profilePictureUrl") or "",
...
async def checknow(interaction: discord.Interaction, pageurl: str):
    ...
    if info:
        await interaction.followup.send(
            f"✅ **{info['title']}** is LIVE — Followers: {info['followers']}"
        )
    else:
        await interaction.followup.send(
